...
def process_case(parallel_param):
    ...
    if t == "static":
        simu.perform_static(index, a_data, b_data, c_data, a_indices, b_indices, c_indices, a_flip, A, A_, B, C, SMALL_DIR_OUTPUT, k, t, myact)
        ...
        simu.perform_simu(index, dp_stochastic, EXIOBASE_AGGREGATED_OUTPUT, k, myact, t, u)
        print(f"{t}_{u} simulation is done.")
    elif t == "log-normal":
        dp_stochastic = simu.add_uncertainty(t, u, a_data, b_data, c_data, a_indices, b_indices, c_indices, a_flip)
        simu.perform_simu(index, dp_stochastic, EXIOBASE_AGGREGATED_OUTPUT, k, myact, t, u)
...
if __name__ == "__main__":
    ...
    for t in DIST_TYPE:
        if t == "static":
            for myact, index in SELECTED_AGGREGATED:
                k += 1
